...
public class FindInfoPacker {
    ...
    //  Enable file id packing for the directory/full-info levels. The FileIndex field
    //  of each entry doubles as the resume key when the client sets SMB_FIND_RETURN_RESUME_KEYS
    //  (macOS's kernel smbfs does for level 0x104). Writing all-zero FileIndex values makes
    //  macOS fall back to per-file lookups and crawl to a halt on large directories, so pack
    //  the driver-provided file id (a stable per-path hash) instead. See PATCHES.md.
